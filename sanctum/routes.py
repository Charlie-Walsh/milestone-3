...
def authors():
    ...
    authors = list(Author.query.order_by(Author.author_lname).all())       
...
def books():
    if request.method == 'POST':
        db.session.add(Title(book_title = request.form.get('book_title'),
                             author_id = request.form.get('author_id')))
        db.session.commit()
        return redirect(url_for('books'))
    books = list(Title.query.order_by(Title.author_id).all())
    authors = list(Author.query.order_by(Author.author_lname).all())       
...
def reviews():
    ...
    reviews = list(Review.query.order_by(Review.title_id).all())
    if request.method == 'POST':
        db.session.add(Review(review = request.form.get('review'),
                              rating = request.form.get('rating'), title_id = request.form.get('book_id')))
        db.session.commit()
        return redirect(url_for('reviews'))
